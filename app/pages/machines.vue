      ...
      <div v-for="m in filteredMachines" :key="m.vmid"
        ...
        <div class="px-4 py-3 border-b border-border bg-muted-surface/30 flex justify-between items-center">
          ...
          <div class="flex items-center gap-1 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
            <button @click="openConsole(m)" title="Abrir Consola"
            ...
            <button @click="openEdit(m)" title="Configuración"
        ...
        <div class="p-6 space-y-4">
          ...
          <form v-else @submit.prevent="handleClone" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input v-model="cloneForm.name" type="text"
                class="w-full p-2.5 bg-background border border-border rounded-lg focus:ring-2 focus:ring-primary/50 text-text placeholder:text-text-muted/50"
                required />
            </div>
            <div>
...
const { isAuthenticated, restoreSession, listVMResources, listPools, listNodes, updateMachineConfig, deleteMachine, cloneMachine, addVmToPool, setMachineStatus, listStorages, listStorageContent, createQemu, createLxc } = useProxmox()
...
// -- Actions --
const changePowerState = async (m: Machine, a: any) => { actionLoading.value = m.vmid; await setMachineStatus(m.node, m.type, m.vmid, a); setTimeout(() => { loadMachines(); actionLoading.value = null }, 2000) }
const openConsole = (m: Machine) => router.push(`/console/${m.vmid}?node=${m.node}&type=${m.type}`)

const openCreateModal = () => { showCreateModal.value = true; createMode.value = 'new' }
...
// Edit handlers
const openEdit = (m: Machine) => { editingMachine.value = m; editForm.value = { name: m.name || '', pool: m.pool || '' } }
